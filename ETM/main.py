...
parser.add_argument('--num_words', type=int, default=11, help='number of words for topic viz')
...
def evaluate(m, source, tc=False, td=False):
    ...
    with torch.no_grad():
        ...
        all_indices = torch.split(torch.tensor(range(len(all_data_counts))),args.eval_batch_size)
        all_theta =[]
...
if args.mode == 'train':
    ...
    r = int(args.dataset.split("_")[0])
    dname = args.dataset.split("_")
    saved_dir = ETM_dir+'/SavedOutput/'+dname[1]+"/"+dname[2]+"/topics_"+str(args.num_topics)+"/runs_"+str(r)
    os.makedirs(saved_dir,exist_ok=True)
    os.chdir(saved_dir)
    torch.save(model.state_dict(),args.dataset+"_"+str(args.num_topics)+".pt")

    all_results = {} 
           
    with open("results_"+args.dataset+"_"+str(args.num_topics)+".txt","a") as f:
      beta_np = beta.data.cpu().numpy()
      theta_all_np = np.asarray(all_theta)
      # theta_all_np = all_theta.data.cpu().numpy()
      ...
      all_results['all_theta'] = theta_all_np
      all_results['beta'] = beta_np
      all_results['seed'] = seed
      
      # compressed_pickle(theta_all_np,'ETM_theta_all_'+args.dataset+"_"+str(args.num_topics))
      # compressed_pickle(beta_np,'ETM_beta_'+args.dataset+"_"+str(args.num_topics))
     
      compressed_pickle(all_results,'ETM_all_results_'+args.dataset+"_"+str(args.num_topics))
      gammas = beta
      topics_words = []
      for k in range(args.num_topics):
          gamma = gammas[k]
          ...
          topic_words = [vocab[a] for a in top_words]
          topics_words.append(' '.join(topic_words))
          f.write('Topic :'+str(k)+" >> "+str(topic_words)+"\n")
      # f.write("\n\n\n\n")
    os.chdir(ETM_dir)
