...
from wtm import Model as wtm_Model
...
parser.add_argument('-s','--smoothen', type=int, default=1e-30, help='error/term-smoothening constant')
...
parser.add_argument('-vd','--var_dropout', type=str, default='True', help='Variational Dropout use - yes/no')
...
parser.add_argument('-drop','--dropout', type=float, default=0.1, help='dropout rate on the encoder')
parser.add_argument('-wdecay','--weight_decay', type=float, default=1.2e-6, help='some l2 regularization')
...
if __name__ == '__main__':
  ...
  if data_name == 'bbc': max_features = 2000
  elif data_name == 'searchsnippet': max_features = 3000
  elif data_name == 'yahooanswers': max_features = 4000
  elif data_name == 'agnews120k': max_features = 8000
  ...
  if(len(data_preprocessed)>100000):
  ...
  if str(have_KLD) == "True": kld_folder = "kld"
  ...
  print('args: '+str(args)+"\n\n")
  print("var-drop:",str(var_dropout),"kld: ",str(have_KLD),"dropout:",dropout,"\n\n")
  ...
  variance_x, bs, embedding_tensor_words,emb_size,smoothen,isbn_muz,isbn_x,isbn_phi,have_KLD,eps_samples, "gaussian")
  ...
  os.chdir(home_dir)
  save_dir = home_dir+"/SavedOutput/"+var_folder+"/"+kld_folder+"/"+data_name+"/"+dtype+"/topics_"+str(args.num_topics)+"/runs_"+str(args.run)
